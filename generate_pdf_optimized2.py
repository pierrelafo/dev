...
def generate_pdf_with_title_and_comments(input_excel_path, first_page_text, image_path,text_encadre, pdf_name):
	...
	for idx, question in questions_to_plot.items():
		...
		if pd.notna(current_category) and current_category.strip() != "":
			...
			if current_category != previous_category and section_data:
				...
				avg_auto_eval = np.mean([score[0] for score in section_data.values()])
				avg_eval = np.mean([score[1] for score in section_data.values()])
				aggregated_data[previous_category] = (avg_auto_eval, avg_eval)
				section_data = {}
	...
	# Générer un graphique unique pour toutes les catégories
	y_position = generate_overall_bar_chart(aggregated_data, c, page_width, y_position, image_height, custom_color1, custom_color2, add_header_image, y_position_init)
	...
	style.textColor = colors.HexColor("#61A6AB") 
	paragraph_width = 490  
	text_open = "Les commentaires repris dans cette section ont été écrits tels quels par les personnes sollicitées. Ils n'ont subi aucune modification, n'ont pas été triés ou classés d'une quelconque manière. " \
				"Si certains commentaires apparaissent plusieurs fois, cela signifie que plusieurs personnes ont fait le même commentaire.<br/><br/><br/>"
	paragraph = Paragraph(text_open, style)
	paragraph_width, paragraph_height = paragraph.wrap(paragraph_width, 0)
	paragraph.drawOn(c, 50, y_position-paragraph_height)
	y_position -= (paragraph_height+10)

	#	#  Debut réponses forces	#	#	
	style.fontSize = 12  
	style.textColor = colors.HexColor("#365B6D") 
	paragraph_width = 490  
	text_open = "<b>Selon-vous, quelles sont ses forces ?</b>"
	paragraph = Paragraph(text_open, style)
	paragraph_width, paragraph_height = paragraph.wrap(paragraph_width, 0)
	paragraph.drawOn(c, 50, y_position-paragraph_height) 
	y_position -= (paragraph_height+10)
	...
	for comment in forces:
		...
		if y_position < 80:  # Sauter à une nouvelle page si l'espace est insuffisant
			c.showPage()
			add_header_image(c)
			y_position = y_position_init

	#	#  Debut réponses amelioration	#	#	
	y_position -= 20
	style.fontSize = 12 
	style.textColor = colors.HexColor("#365B6D") 
	paragraph_width = 490
	text_open = "<b>Selon-vous, quelles sont ses axes d'amelioration ?</b>"
	paragraph = Paragraph(text_open, style)
	paragraph_width, paragraph_height = paragraph.wrap(paragraph_width, 0)
	paragraph.drawOn(c, 50, y_position-paragraph_height)
	y_position -= (paragraph_height+10)
	...
	style.textColor = colors.HexColor("#365B6D") 
	paragraph_width = 490
	text_open = "<b>En termes d’évolution professionnelle, quels pourraient être ses domaines de réussite ?</b>"
	paragraph = Paragraph(text_open, style)
	paragraph_width, paragraph_height = paragraph.wrap(paragraph_width, 0)
	paragraph.drawOn(c, 50, y_position-paragraph_height)
	y_position -= (paragraph_height+10)
	
	evolutions = sheet_2.iloc[:, evolution_col].dropna().tolist()
	style.fontSize = 10
	style.textColor = colors.HexColor("#61A6AB") 
	for comment in evolutions:
		paragraph = Paragraph("&nbsp;&nbsp;&nbsp;&nbsp;- " + comment, style)
		paragraph_width, paragraph_height = paragraph.wrap(paragraph_width, 0)
		paragraph.drawOn(c, 50, y_position-paragraph_height) 
		y_position -= (paragraph_height+5)
...
def generate_overall_bar_chart(aggregated_data, c, page_width, y_position, image_height, custom_color1, custom_color2, add_header_image, y_position_init):
	...
	ax.barh(indices , evaluator_scores, height=bar_width, label="Évaluateurs", color=custom_color2, zorder=2)
	ax.barh(indices + bar_width, auto_evaluation_scores, height=bar_width, label="Auto-évaluation", color=custom_color1, zorder=2)
...
# Exemple d'appel
input_excel_path = r"D:\Drive LafontaineConsulting Microsoft\OneDrive - Lafontaine Consulting\Nouveau\xls_to_pdf\fichier_excel.xlsx"
...
#generate_pdf_with_title_and_comments(input_excel_path, first_page_text, image_path,text_encadre, pdf_name)
